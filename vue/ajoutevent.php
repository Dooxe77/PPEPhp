...
<center>
			...
			// Traitement de l'envoi de l'événement
				...
				// Traitement de l'enregistrement de l'événement
					...
					if (date('l', $timeDuJour) == 'Saturday')
					{
						...
						echo '<ul><li>Une réservation ne peut commencer que la samedi </li></ul>';
						}
			}
   ...
   ?> 
    ...
    <form method="get" action="ajoutevent.php?type='.$typeLogement.'" name="form1" id="formulaire">
    	<table id="tabAjoutEvent">
        	<tr>
				<td><label for="type">Type de prestation souhaité: </label>
				<?php
				echo"<select name='type' onChange='form1.submit();'>";
				echo"<option>----------------------------</option>";
...
?>
		...
		<form method="post" action="ajoutevent.php" name="form2" id="formulaire">			
				<table id="tabAjoutEvent">
			</tr>
			<tr>
...
<?php
	if(isset($_GET['type']))
		{
			$_SESSION['level'] = htmlentities($_GET['type']);
			if ( $num_Logement == "" )
				{
